# Block Zero Core

Block Zero Core is the chain implementation for the Block Zero network.

Block Zero is not Bitcoin. It is an independent, Bitcoin-inspired fair-launch network.

## Principles

- No ICO
- No premine
- No VC allocation
- Open source
- Mineable from genesis

## Status

Early-stage fork. The upstream baseline is pinned to Bitcoin Core `v31.0`
(see [UPSTREAM.md](UPSTREAM.md)). Source import and chain-identity changes are
the next steps.

## Disclaimer

Block Zero is an experimental open-source blockchain project. BLOZ has no guaranteed value, no promised liquidity and no expected return. Participation is voluntary and mainly intended for technical, educational and community purposes.
