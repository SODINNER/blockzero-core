...
public:
    CMainParams() {
        ...
        //
        // Activation is set to 413, the block right after the current tip (412),
        // via a coordinated flag day: the network is paused, every miner upgrades,
        // then mining resumes. Blocks 0..412 already exist and were mined without
        // SegWit, so activation MUST be strictly above the tip (413) to keep them
        // valid. SegWit then enforces from block 413, which protects all existing
        // P2WPKH (bz1) coinbase outputs from anyone-can-spend theft (coinbase
        // maturity is 100 blocks, so none can be spent before SegWit is active).
        // All nodes MUST upgrade before block 413 or the network will split.
        ...
        // Block Zero mainnet genesis (RandomX proof-of-work). Launch 2026-06-06 06:06:06 UTC.
        {
            const char* bz_genesis_msg = "The Times 06/Jun/2026 Block Zero - a second chance at Genesis";
            ...
            genesis = CreateGenesisBlock(bz_genesis_msg, bz_genesis_script, 1780725966, 632005, 0x1e3fffff, 1, 50 * COIN);
        }
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256{"44c1a8c852b3eda21966e1ddb6b0807e22488dffe8a270bf24bf1fa2d66c13bd"});
        assert(genesis.hashMerkleRoot == uint256{"1de902b6ecd142366a9d81f27bc1a60457a56761b7bf6aafc1a6a33c3a49b005"});
        ...
        // Block Zero: fresh chain — no assumeutxo snapshots yet.
        m_assumeutxo_data = {};

        chainTxData = ChainTxData{
            // Block Zero: fresh chain, no historical tx statistics yet.
            .nTime    = 0,
            .tx_count = 0,
            .dTxRate  = 0,
        };
...
public:
    CTestNetParams() {
        ...
        // Block Zero testnet genesis (RandomX proof-of-work), 2026-06-04.
        {
            const char* bz_genesis_msg = "The Times 04/Jun/2026 Block Zero - a second chance at Genesis";
            const CScript bz_genesis_script = CScript() << "04678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38c4f35504e51ec112de5c384df7ba0b8d578a4c702b6bf11d5f"_hex << OP_CHECKSIG;
            genesis = CreateGenesisBlock(bz_genesis_msg, bz_genesis_script, 1780531200, 244191, 0x1e3fffff, 1, 50 * COIN);
        }
        consensus.hashGenesisBlock = genesis.GetHash();
